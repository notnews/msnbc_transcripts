"""Build the Parquet deliverable from every source the corpus has.

Inputs, by file type:

* ``*.jsonl``: checkpoints written by ``msnbc-transcripts scrape``.
* ``*_metadata.csv`` with an ``id`` column: the 2026 API scrape's metadata
  file. Its text lives in ``{id}.html.gz`` files next to it (``--html-dir``),
  which are parsed here so the Parquet carries text, not just metadata.
* other ``*.csv`` with a ``url`` column: the 2025 listing-page scrape
  (``air_date, show_name, headline, guests, url, ..., summary, text``).

Rows are deduplicated on ``url``; the first occurrence wins.
"""
...
from msnbc_transcripts.parsers import parse_rendered_html
...
def rows_from_api_metadata(path: Path, html_dir: Path) -> list[dict[str, Any]]:
    """Join the API metadata CSV to its ``{id}.html.gz`` files."""
    from msnbc_transcripts.parsers import _program_from_api_title, normalize_space
    ...
    with (
        ...
        for row in csv.DictReader(handle):
            html_path = html_dir / f"{row['id']}.html.gz"
            summary, text = None, None
            if html_path.exists():
                with gzip.open(html_path, "rt", encoding="utf-8") as raw:
                    summary, text = parse_rendered_html(raw.read())
            title = normalize_space(row.get("title") or "") or None
            records.append(
                {
                    "id": row["id"],
                    "url": row["url"],
                    "title": title,
                    "program": _program_from_api_title(title),
                    "aired_date": _iso_date(row.get("date")),
                    "aired_time": _iso_time(row.get("date")),
...
def load_records(
    paths: list[Path], html_dir: Path | None = None
    ...
    for path in paths:
        ...
        elif path.name.removesuffix(".gz").endswith("_metadata.csv"):
            rows = rows_from_api_metadata(
                path, html_dir or path.parent / "transcripts_html"
            )
        else:
